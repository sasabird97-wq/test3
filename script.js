// V17: Mobile-centered fix (uses dynamic viewport units). Keeps V16 features.
// ---------- TIMER (green digital with slow random fades) ----------
...
function updateWithFade(el){
  ...
  setTimeout(()=>{ el.textContent = randStr(2); el.classList.remove('fade-out'); }, FADE_DURATION/2);
}
function loop(el){
...
let blinkOn = true;
setInterval(()=>{ blinkOn = !blinkOn; seps.forEach(s => s.classList.toggle('off', !blinkOn)); }, 1000);

// ---------- SPACE BACKGROUND (canvas): stars + centered spiral galaxy ----------
const canvas = document.getElementById('space');
const ctx = canvas.getContext('2d');
let Wcss, Hcss, DPR;
let stars = [], galaxy = []; let rot = 0;
...
function genGalaxy(){
  ...
  for(let i=0;i<PARTICLES;i++){
    ...
    const angle = (i % ARMS) * armSep + t * 1.6 + (Math.random() - 0.5) * 0.25;
...
function resize(){
  ...
  Wcss = window.innerWidth; Hcss = window.innerHeight;
...
function draw(){
  ...
  ctx.beginPath(); ctx.arc(cx, cy, Math.min(Wcss, Hcss)*0.36, 0, Math.PI*2); ctx.fill();
  rot += 0.002;
  const cos = Math.cos(rot), sin = Math.sin(rot);
  for(const p of galaxy){
    const gx = p.x * cos - p.y * sin, gy = p.x * sin + p.y * cos;
    ctx.globalAlpha = p.alpha; ctx.fillStyle = '#cffff0';
    ctx.beginPath(); ctx.arc(cx + gx, cy + gy, p.size, 0, Math.PI*2); ctx.fill();
  }
  requestAnimationFrame(draw);
}
draw();

// ---------- WEBAUDIO: Always-on space ambience (fixed volume, auto-resume) ----------
let actx = null, masterGain, noiseSrc, noiseGain, lp, bp, lfo, lfoGain, rumbleOsc, rumbleGain, delayL, delayR, fbL, fbR, merger;
function createNoiseBuffer(ctx, seconds=2){
  const sr = ctx.sampleRate, buffer = ctx.createBuffer(1, sr*seconds, sr);
...
function setupAudio(){
  ...
  noiseSrc = actx.createBufferSource(); noiseSrc.buffer = noiseBuffer; noiseSrc.loop = true;
  noiseGain = actx.createGain(); noiseGain.gain.value = 0.35;
  lp = actx.createBiquadFilter(); lp.type = 'lowpass'; lp.frequency.value = 800; lp.Q.value = 0.7;
  bp = actx.createBiquadFilter(); bp.type = 'bandpass'; bp.frequency.value = 400; bp.Q.value = 1.2;
  lfo = actx.createOscillator(); lfo.frequency.value = 0.05;
  lfoGain = actx.createGain(); lfoGain.gain.value = 200; lfo.connect(lfoGain).connect(bp.frequency);
  rumbleOsc = actx.createOscillator(); rumbleOsc.type = 'sine'; rumbleOsc.frequency.value = 48;
  rumbleGain = actx.createGain(); rumbleGain.gain.value = 0.03;
  delayL = actx.createDelay(3.0); delayL.delayTime.value = 0.27;
  delayR = actx.createDelay(3.0); delayR.delayTime.value = 0.41;
  fbL = actx.createGain(); fbL.gain.value = 0.25; delayL.connect(fbL).connect(delayL);
  fbR = actx.createGain(); fbR.gain.value = 0.21; delayR.connect(fbR).connect(delayR);
  merger = actx.createChannelMerger(2);
  const dryGain = actx.createGain(); dryGain.gain.value = 0.05;
  noiseSrc.connect(lp); lp.connect(bp); bp.connect(delayL); bp.connect(delayR); bp.connect(dryGain).connect(masterGain);
  rumbleOsc.connect(rumbleGain); rumbleGain.connect(delayL); rumbleGain.connect(delayR);
  delayL.connect(merger, 0, 0); delayR.connect(merger, 0, 1); merger.connect(masterGain);
  noiseSrc.start(); lfo.start(); rumbleOsc.start();
}
...
['pointerdown','click','keydown','touchstart'].forEach(evt => { window.addEventListener(evt, ensureAudio, {passive:true}); });
